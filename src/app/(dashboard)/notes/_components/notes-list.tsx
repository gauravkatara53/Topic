...
export function NotesList({ initialFiles }: { initialFiles: any[] }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center w-full">
                ...
                <div className="flex flex-wrap gap-4">
                    <Select value={ownership} onValueChange={setOwnership}>
                        <SelectTrigger className="w-[130px] h-10 border-slate-200 rounded-xl bg-white shadow-sm font-medium text-slate-600 text-sm">
                    ...
                    <Select value={semester} onValueChange={setSemester}>
                        <SelectTrigger className="w-[140px] h-10 border-slate-200 rounded-xl bg-white shadow-sm font-medium text-slate-600 text-sm">
                    ...
                    <Select value={type} onValueChange={setType}>
                        <SelectTrigger className="w-[150px] h-10 border-slate-200 rounded-xl bg-white shadow-sm font-medium text-slate-600 text-sm">
            ...
            <div className="bg-gradient-to-r from-[#1e3a5f] to-[#20948b] rounded-2xl p-6 text-white shadow-sm relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 w-full">
                    {displayTopFiles.map(n => (
                        <div key={n.rank} className={`bg-white/10 rounded-xl p-4 border border-white/5 backdrop-blur-sm ${n.isEmpty ? 'opacity-50' : ''}`}>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {filteredFiles.length === 0 ? (
                    <div className="col-span-full text-center py-12 text-slate-500 bg-white rounded-2xl border border-slate-200 shadow-sm border-dashed">
                        No files found matching your criteria.
                    </div>
                ) : (
                    filteredFiles.map((file) => (
                        <Card key={file.id} className="group hover:shadow-md transition-shadow rounded-2xl border-slate-200/60 shadow-sm p-6 bg-white overflow-hidden">
                            <div className="flex justify-between items-start mb-4">
                                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-semibold uppercase tracking-wider ${file.type === 'PYQ' ? 'bg-orange-50 text-orange-500' : 'bg-teal-50 text-teal-600'}`}>
                            ...
                            <div className="space-y-1 mb-6">
                                <h3 className="text-lg font-bold text-slate-800 line-clamp-1">{file.subject}</h3>
                                <p className="text-sm text-slate-500">
                                    {file.branch} · {file.semester}{file.semester === 1 ? 'st' : file.semester === 2 ? 'nd' : file.semester === 3 ? 'rd' : 'th'}
                                </p>
                                <div className="flex justify-between items-center mt-0.5">
                                    <p className="text-xs text-slate-400">by {file.uploader?.name || "Anonymous"}</p>
                                    {user?.id === file.uploaderId && (
                                        <div className="flex gap-1">
                                            <Button size="icon" variant="ghost" className="h-6 w-6 text-slate-400 hover:text-blue-600" onClick={() => setEditingFile(file)}>
                                                <Pencil className="h-3 w-3" />
                                            </Button>
                                            <Button size="icon" variant="ghost" className="h-6 w-6 text-slate-400 hover:text-red-600" onClick={() => handleDelete(file.id)}>
                            ...
                            <div className="flex justify-between items-center mt-4 pt-4 border-t border-slate-100">
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        size="icon"
                                        variant="ghost"
                                        className={`h-8 w-8 transition-colors ${likedFiles.has(file.id) ? 'text-teal-600 bg-teal-50' : 'text-slate-400 hover:text-teal-600 hover:bg-teal-50'}`}
                                        onClick={() => handleRate(file.id)}
                                        disabled={likedFiles.has(file.id)}
                                    >
                                        <ThumbsUp className={`h-4 w-4 ${likedFiles.has(file.id) ? 'fill-current' : ''}`} />
                                    </Button>
                                    <Button size="icon" variant="ghost" className="h-8 w-8 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50" onClick={() => handleShare(file)}>
