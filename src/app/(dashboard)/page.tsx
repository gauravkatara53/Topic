...
import { ArrowRight, Bell, Calendar as CalIcon, BookOpen, ShieldAlert, CheckCircle, AlertTriangle, ClipboardCheck, LogIn } from "lucide-react";
...
export default async function Home() {
  ...
  // Fetch public data (announcements, events, notes) always
  // Fetch user-specific data (attendance) only when logged in
  const [dbAnnouncementsRaw, events, notes, attendance] = await Promise.all([
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto pb-8 sm:pb-10 font-sans">
        ...
        <div className="relative z-10 space-y-1.5 sm:space-y-2">
          ...
          <p className="text-white/80 flex items-start sm:items-center gap-2 mt-2 sm:mt-4 text-[13px] sm:text-[15px] leading-snug">
            {isGuest ? (
              ...
              <span className="opacity-90 flex items-center gap-1.5"><CalIcon className="w-3.5 h-3.5 mt-0.5" /> {events[0].title} on {format(new Date(events[0].date), "MMM d")}</span>
            ) : (
              <span className="opacity-90">Enjoy your day! No upcoming events.</span>
            )}
          </p>
        </div>
      </div>
        ...
        <Card className="shadow-sm border-slate-200/50 rounded-2xl overflow-hidden min-h-[120px] sm:h-[164px]">
          <CardContent className="p-3.5 sm:p-5 flex flex-col justify-between h-full gap-3 sm:gap-0">
            ...
            <div className="space-y-0.5 mt-auto">
              ...
              <p className="text-[11px] sm:text-[13px] text-slate-500 font-medium pt-1 line-clamp-2 leading-tight">
                Notices <span className="text-slate-400 font-normal">· {announcements.length === 0 ? "All caught up" : "Recently"}</span>
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      {!isGuest && attendance.length > 0 && globalAttendance < 75 && (
      ...
      {/* Upcoming Events Container matching 3rd screenshot */}
