        ...
        <div className="space-y-6 max-w-4xl mx-auto pb-10 font-sans">
            <div className="relative">
                ...
                <div className="absolute -bottom-16 left-8 flex items-end gap-6 w-full pr-16 justify-between">
                    <Avatar className="h-32 w-32 border-4 border-white dark:border-slate-900 shadow-md bg-white dark:bg-slate-800">
                        <AvatarImage src={user.imageUrl || ""} alt={name} />
                        <AvatarFallback className="text-4xl bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400 font-bold">
                            {userInitials}
                        </AvatarFallback>
                    </Avatar>
            ...
            <div className="pt-20 px-4">
                <div className="flex justify-between items-start">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{name}</h1>
                        {branch && semester ? (
                    ...
                    <Badge className="bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 hover:bg-teal-200 dark:hover:bg-teal-500/30 border-none px-4 py-1 flex gap-1.5 items-center">
                        <span className="h-2 w-2 rounded-full bg-teal-500 dark:bg-teal-400 shrink-0"></span>
                        Active Student
                    </Badge>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                    <Card className="col-span-2 shadow-sm border-slate-200/60 dark:border-slate-700/60 dark:bg-slate-800 rounded-2xl">
                        <CardHeader>
                            <CardTitle className="text-lg text-slate-800 dark:text-slate-100">About</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <p className="text-slate-600 dark:text-slate-300 leading-relaxed text-[15px] whitespace-pre-wrap">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-2 text-[14px]">
                                ...
                                <div className="flex items-center gap-3 text-slate-600 dark:text-slate-300">
                                    ...
                                    <span className="truncate">Roll No: <span className="font-medium text-slate-800 dark:text-slate-200">{rollDisplay}</span></span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>

                    <Card className="shadow-sm border-slate-200/60 dark:border-slate-700/60 dark:bg-slate-800 rounded-2xl h-max">
                        <CardHeader>
                            <CardTitle className="text-lg text-slate-800 dark:text-slate-100">Quick Stats</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-700/60">
                                ...
                                <span className={`font-bold ${userDb.cgpa ? 'text-slate-800 dark:text-slate-200' : 'text-slate-300 dark:text-slate-600 font-normal text-sm'}`}>
                            ...
                            <div className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-700/60">
                                ...
                                <span className={`font-bold ${currentAttendance !== "N/A" ? 'text-orange-500 dark:text-orange-400' : 'text-slate-300 dark:text-slate-600 font-normal text-sm'}`}>
                                    {currentAttendance}
                                </span>
                            </div>
                            <div className="flex justify-between items-center py-2">
                                <span className="text-slate-500 dark:text-slate-400 font-medium">Notes Shared</span>
                                <span className="font-bold text-teal-600 dark:text-teal-400">{uploadedFiles}</span>
                            </div>
                        </CardContent>
                    </Card>
                </div>
